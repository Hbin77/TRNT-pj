...
export default function NewScenarioPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await scenarioAPI.generateStream(
        requestData,
        (content) => {
          setStreamedText((prev) => prev + content);
        },
        (scenarioId) => {
          // 완료 시 2초 후 상세 페이지로 이동
          setTimeout(() => {
            router.push(`/scenarios/${scenarioId}`);
          }, 2000);
        },
        (errorMessage) => {
          alert(errorMessage || '시나리오 생성에 실패했습니다.');
        ...
        <div className="max-w-3xl mx-auto">
          ...
          {isStreaming && (
            <motion.div
              ...
              className="mb-8"
            >
              <GlassCard className="border border-blue-500/20 bg-blue-900/5 backdrop-blur-xl">
                <div className="flex items-center space-x-3 mb-6">
                  <div className="bg-blue-500/20 p-2 rounded-lg border border-blue-500/30">
                    <Sparkles className="w-5 h-5 text-blue-400 animate-pulse" />
                  </div>
                  <div>
                    <h2 className="text-lg font-bold text-white">평행세계가 펼쳐지고 있습니다...</h2>
                    <p className="text-xs text-white/40">실시간으로 시나리오가 생성됩니다</p>
                  </div>
                </div>
                <div
                  ref={streamRef}
                  className="max-h-[60vh] overflow-y-auto prose prose-invert prose-lg max-w-none"
                >
