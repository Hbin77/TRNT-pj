...
- 원래 했던 선택 (참고용): {branch.original_choice}
</branch_info>

**[시나리오 전제]**
주인공은 위 시점에서 "{branch.original_choice}" 대신 **"{branch.alternative_choice}"**를 선택했습니다.
이 시나리오는 **"{branch.alternative_choice}"를 선택한 이후 펼쳐지는 인생**을 그립니다.
...
1. **시나리오 전제 준수**: 위 [시나리오 전제]에 명시된 선택을 이야기의 출발점으로 삼으십시오.
   주인공은 이미 그 선택을 했고, 그 이후의 인생을 그리는 것입니다.
...
**시나리오 시작점**: 주인공이 "{branch.alternative_choice}"를 선택한 직후부터 이야기를 시작하십시오.
