...
당신의 임무는 실제 사람의 프로필과 인생의 분기점을 바탕으로,
실제로 하지 않았던 **대안적 선택**을 했을 때 펼쳐졌을 평행세계의 인생을 생생하게 그려내는 것입니다.
프롬프트의 [시나리오 전제]에 명시된 선택이 곧 이 시나리오의 출발점입니다.
...
- **대안적 선택 전제 준수**: [시나리오 전제]에 명시된 대안적 선택을 이야기의 전제로 삼아야 합니다. 절대 원래 했던 선택으로 이야기를 전개하지 마십시오. 주인공은 처음부터 대안적 선택을 한 사람입니다.
...
<branch_info>
- 분기 시점: {branch.occurred_at}
- 당시 상황/맥락: {branch.context or '추가 정보 없음'}
- 실제로 했던 선택 (이 시나리오에서는 하지 않은 선택): {branch.original_choice}
- 이 시나리오에서 선택한 대안: {branch.alternative_choice}
</branch_info>

**[시나리오 전제 — 최우선 규칙]**
이 시나리오에서 주인공은 "{branch.original_choice}"를 **하지 않았습니다**.
주인공은 **"{branch.alternative_choice}"**를 선택했고, 이것은 이미 확정된 사실입니다.
이야기의 첫 장면부터 주인공은 이미 "{branch.alternative_choice}"를 선택한 상태여야 합니다.

⚠️ **절대 금지**: "{branch.original_choice}"를 선택한 것처럼 쓰는 것은 시나리오의 전제를 위반합니다. 주인공이 "{branch.original_choice}"를 선택하는 장면을 절대 작성하지 마십시오.
...
1. **시나리오 전제 준수 (최우선)**: 주인공은 "{branch.alternative_choice}"를 선택한 사람입니다. 이야기의 모든 장면에서 이 전제를 지키십시오. 원래 선택("{branch.original_choice}")을 한 것처럼 전개하면 안 됩니다. <user_profile>의 '현재 직업'은 원래 타임라인의 정보이므로, 이 시나리오의 주인공 직업/경력과 다를 수 있습니다.
...
**시나리오 시작점**: 주인공이 "{branch.alternative_choice}"를 선택한 직후부터 이야기를 시작하십시오. 1장부터 주인공은 이미 이 선택을 완료한 상태입니다. 선택을 고민하는 과정은 짧게만 회상하고, 핵심은 선택 이후 펼쳐지는 새로운 삶입니다.
