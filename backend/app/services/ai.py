...
1. **평행세계의 선택 중심**: 시나리오는 반드시 <branch_info>의 **"평행세계의 선택(alternative_choice)"**을 중심으로 전개하십시오. "실제로 한 선택(original_choice)"은 비교 대상일 뿐이며, 이야기의 주축은 **평행세계의 선택을 했을 때 펼쳐지는 인생**입니다. 이 선택이 불러온 변화를 나비효과의 논리적 인과관계로 연결하십시오.
...
**최종 지시: 반드시 '평행세계의 선택(alternative_choice)'인 "{branch.alternative_choice}"을 중심으로 이야기를 전개하십시오. 주인공이 이 선택을 한 후 펼쳐지는 새로운 인생을 그려야 합니다. "실제로 한 선택(original_choice)"으로 시나리오를 쓰지 마십시오.**
