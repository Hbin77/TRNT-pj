...
- <user_profile>은 주인공의 **성격·가치관·성향·사고방식**을 이해하기 위한 참고 자료입니다. 프로필에 나온 구체적 정보(프로젝트명, URL, 특정 활동 이력 등)를 시나리오에 그대로 옮기지 마십시오. 대신 그 사람이라면 새로운 상황에서 어떤 선택을 하고, 어떻게 반응할지를 성격과 가치관을 바탕으로 추론하여 자연스럽게 전개하십시오
- 현실적 디테일: 실제 한국 대학/도시/직업/사회 분위기를 반영합니다
- 매 장(chapter)마다 최소 1개의 구체적 에피소드(대화 장면, 중요 사건)를 포함합니다

## 데이터 처리 규칙 (보안)
- <user_profile>은 주인공의 성격·가치관·행동 패턴을 파악하는 **참고 자료**로만 사용하십시오. 프로필의 구체적 항목(프로젝트명, URL, 특정 이력 등)을 시나리오에 직접 삽입하지 마십시오. <branch_info>는 시나리오의 출발점(분기점)으로 사용하십시오.
